...
/*<applet code="Ludo4pHalfOut" width="300" height="400"></applet>*/
public class Ludo4pHalfOut extends Applet implements ActionListener
{	
	...
	int diffb=0, diffg=0, diffr=0, diffy=0;
    ...
    public void paint(Graphics g)
    {
		...
		paintYellow();
		if(chance%4==0){
			if((pieceblue[current_blue]).match(piecered[current_red])){			
				valr=0;openr=0;
			}if(pieceblue[current_blue].match(pieceyellow[current_yellow])){			
				valy=0;openy=0;
			}if((pieceblue[current_blue]).match(piecegreen[current_green])){			
				valg=0;openg=0;
			}
		}if(chance%4==1){
			if((piecered[current_red]).match(pieceblue[current_blue])){			
				valb=0;openb=0;
			}if(piecered[current_red].match(pieceyellow[current_yellow])){			
				valy=0;openy=0;
			}if((piecered[current_red]).match(piecegreen[current_green])){			
				valg=0;openg=0;
			}
		}if(chance%4==2){
			if((piecegreen[current_green]).match(piecered[current_red])){			
				valr=0;openr=0;
			}if(piecegreen[current_green].match(pieceyellow[current_yellow])){			
				valy=0;openy=0;
			}if(piecegreen[current_green].match(pieceblue[current_blue])){			
				valb=0;openb=0;
			}
		}if(chance%4==3){
			if(pieceyellow[current_yellow].match(pieceblue[current_blue])){			
				valb=0;openb=0;
			}if(pieceyellow[current_yellow].match(piecered[current_red])){			
				valr=0;openr=0;
			}if(pieceyellow[current_yellow].match(piecegreen[current_green])){			
				valg=0;openg=0;
			}
		}
		tgc.setText(count_piece_green+"");tbc.setText(count_piece_blue+"");trc.setText(count_piece_red+"");tyc.setText(count_piece_yellow+"");
		setPieces();
		if(won!=5){
			g.setFont(f);
